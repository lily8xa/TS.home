...
function square(aNam:number,bNam:number){
    const result:number=aNam*bNam;
    return result;
}
let p1=square(10,40);
let p2=square(22,44);
...
function CelinderSquare(radius:number,height:number):number {
    return 2* Math.PI*radius*height;
}
console.log(CelinderSquare(2,5));

// debugger
function ArrFilter(user:string){
    let usersName:string[]=[`Anna`,`Ivan`,`Olesya`,`Valentina`]
...
function textLi(text:string):string{
    document.write(`<ul>
<li>${text}</li>
<li>${text}</li>
<li>${text}</li>
...
function TextNumber(text:string,number:number):string{
    document.write(`<ul>`);
...
function innerFromArr(arr:(string | number | boolean)[]):(string | number | boolean)[]{
    document.write(`<ul>`);
    for (let item of arr){
...
function itemUser(users:IUsers[]):IUsers[]{
...
// Приклад  swap([11,22,33,44],0,1) //=> [22,11,33,44]
//@ts-ignore
function swap1(arr:number[],i1:number,i2:number):number[]{
    let temp=arr[i1];
    arr[i1]=arr[i2];
    arr[i2]=temp;
    return arr;
...
function chekCurrency(currencyBase:string[],currency:string):true | undefined{
...
function exchange(sumUAH:number,currencyValues:{currency:string,value:number} [],exchangeCurrency:string):number | string | undefined{

    const currencyBase:string[]=['USD','EUR','GBP']
